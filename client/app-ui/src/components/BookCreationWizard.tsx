...
export function BookCreationWizard({ onCreated, onCancel }: { onCreated: (book: Book) => void; onCancel: () => void }) {
  const [step, setStep] = useState<'book-type' | 'creation-mode' | 'book-details' | 'format-selection' | 'expert-source-setup'>('book-type')
  ...
  async function createBookBase() {
    const created = await api.createBook({
      ...
      layout_template: selectedType.defaultFormat,
      format_settings: { ...formatSettings, selected_layout_id: selectedType.defaultFormat },
    })
    setBook(created)
    if (creationMode === 'expert' && created.project_id) {
      setSources(await api.listSources(created.project_id))
  ...
  if (step === 'book-type') return <section className="panel"><h2>What type of book are you designing?</h2>{BOOK_TYPES.map((t) => <button key={t.id} onClick={() => { setBookTypeId(t.id); setCreationMode(t.defaultMode) }}>{t.displayName} · {t.description} · Recommended: {t.defaultMode}</button>)}<button onClick={() => setStep('creation-mode')}>Next</button><button onClick={onCancel}>Cancel</button></section>
