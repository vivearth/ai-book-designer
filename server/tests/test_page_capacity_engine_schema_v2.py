...
def test_capacity_uses_validation_capacity():
    ...
    page.layout_json = {'layout_schema': 'page-layout-1', 'validation': {'estimated_text_capacity_words': 333}}
...
def test_capacity_uses_text_boxes_without_validation():
    ...
    book = Book(title='t', book_type_id='custom')
    page = Page(page_number=1, book=book)
    page.layout_json = {'layout_schema': 'page-layout-1', 'typography': {'body_size': 11, 'line_height': 1.5}, 'elements': [{'id': 'text_main', 'type': 'text', 'box': {'x': 0, 'y': 0, 'w': 500, 'h': 700}}]}
