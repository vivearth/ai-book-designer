...
def test_book_type_mapping_defaults():
    assert get_book_type_config('fiction_novel').recommended_mode == 'classical'
    assert get_book_type_config('fiction_novel').default_skill == 'fiction_book_page'
    assert get_book_type_config('marketing_story').recommended_mode == 'expert'
    ...
    assert 'classical' in get_book_type_config('case_study_collection').allowed_modes
